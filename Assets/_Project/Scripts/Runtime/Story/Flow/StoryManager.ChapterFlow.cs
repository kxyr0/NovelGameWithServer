...
public partial class StoryManager
{
	...
	IEnumerator StartCurrentChapterRoutine()
	{
		...
		if (NetworkManager.HasCatalogRemoteContent(episodeId) || !HasPlayableLocalChapter())
			yield return PrepareRemoteGraphCacheForEpisodeIfPossible(episodeId);
	...
	void DoStartCurrentChapter(ChapterData chapter)
	{
		...
		CaptureRetryCheckpoints(startNode);
	...
	bool IsChapterUnlocked(int season, int chapter)
	{
		...
		if (!string.IsNullOrEmpty(episodeId) && HasUnlockedEpisode(episodeId))
			return true;
		if (CanOpenChapterBySubscription(chapterData, episodeId))
			return true;
