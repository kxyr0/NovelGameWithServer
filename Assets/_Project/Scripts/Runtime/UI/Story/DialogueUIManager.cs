...
public class DialogueUIManager : MonoBehaviour
{
	...
	Coroutine dialogueLayoutRefreshRoutine;
	bool dialogueLayoutRefreshRequested;
	bool applyingDeferredDialogueLayout;
	int dialoguePreferredSizeVersion;
	int cachedDialoguePreferredSizeVersion = -1;
	float cachedDialoguePreferredWidth = -1f;
	Vector2 cachedDialoguePreferredSize;
	...
	{
		AutoWireRequiredReferences();
		InvalidateDialoguePreferredSize();
	...
	void OnDisable()
	{
		...
		if (dialogueLayoutRefreshRoutine != null)
		{
			StopCoroutine(dialogueLayoutRefreshRoutine);
			dialogueLayoutRefreshRoutine = null;
		}

		dialogueLayoutRefreshRequested = false;
		applyingDeferredDialogueLayout = false;
	}
	...
	void SetDialoguePanelVisible(bool visible)
	{
		...
		bool visibilityChanged = dialoguePanel.gameObject.activeSelf != visible;
	...
	void BeginWardrobeScreenMode(WardrobeHeroSetupPage setupPage = null)
	{
		...
		EnsureWardrobeCategoryTabsActive(activeWardrobeSetupPage);
		wardrobeScreenModeActive = true;
		SetWardrobePanelVisible(true);
		OpenWardrobeNavigationScreen();
		SetDialoguePanelVisible(false);
	}
	...
	void OpenWardrobeNavigationScreen()
	{
		const string wardrobeScreenId = "Wardrobe";
		...
		bool opened = navigator != null && navigator.OpenScreen(wardrobeScreenId);
		if (!opened)
		{
			// Wardrobe is a real screen, not an anonymous overlay. Even without a navigator,
			// keep the global screen state correct so navigation visibility/active-state rules work.
			UIScreenState.SetCurrentScreen(wardrobeScreenId);
			UIScreenState.SetSelectedScreen(wardrobeScreenId);
		}

		AppLogger.Info(
			AppLogCategory.ScreenNavigation,
			nameof(DialogueUIManager),
			nameof(OpenWardrobeNavigationScreen),
			"[SCREEN][WARDROBE] Wardrobe opened as a real navigation screen.",
			LogMetadata.Of(
				"hasNavigator", navigator != null,
				"openedByNavigator", opened,
				"currentScreenId", UIScreenState.CurrentScreenId));
	}
	...
	Button CreateChoiceButton(out GameObject root, bool premiumChoice)
	{
		...
		if (button != null)
		{
			if (usesDedicatedPremiumPrefab)
			{
				...
				PaidChoiceAdaptiveLayout paidLayout = cost != null
					? cost.GetComponent<PaidChoiceAdaptiveLayout>()
					: null;
				if (paidLayout == null && cost != null)
					paidLayout = cost.gameObject.AddComponent<PaidChoiceAdaptiveLayout>();

				if (paidLayout != null)
					paidLayout.Configure(button);
				else
					Debug.LogError("[DialogueUIManager] Premium choice prefab must contain Button/Cost with CostText and Image children.", root);
			}
			else
	...
	{
		string safeSpeaker = speakerName ?? "";
		string safeBody = bodyText ?? "";

		if (nameText != null && nameText.text != safeSpeaker)
			nameText.SetText(safeSpeaker);

		if (dialogueText != null && dialogueText.text != safeBody)
		{
			dialogueText.SetText(safeBody);
			InvalidateDialoguePreferredSize();
		}
		...
		{
			growDownLock.MarkDirty();
			growDownLock.ApplyIfDirtyNow();
		}
		...
		RequestDialogueLayoutRefresh();
		ApplyNamePlateVisibility(safeSpeaker);
		ApplyActiveCharacterNameLayout();
	}
	...
	bool CanAnimateTextChange()
	{
		return animateTextChanges &&
			   ...
			   (textFadeOutDuration > 0f ||
				textFadeInDuration > 0f);
	}
	...
	void SetDialogueTextAlpha(float alpha)
	{
		...
		if (nameText != null && !Mathf.Approximately(nameText.alpha, alpha))
			nameText.alpha = alpha;

		if (dialogueText != null && !Mathf.Approximately(dialogueText.alpha, alpha))
	...
	$"DOTween.timeScale={DOTween.timeScale}, " +
	$"ActiveTweens={DOTween.TotalActiveTweens()}, " +
	$"PlayingTweens={DOTween.TotalPlayingTweens()}, " +
	$"GameObjectActive={gameObject.activeInHierarchy}, " +
	$"CanAnimate={CanAnimateTextChange()}",
	this);
		EnsureStoryUiVisible();
	...
	void RequestDialogueLayoutRefresh()
	{
		if (!Application.isPlaying)
		{
			ClampDialoguePanelToSafeArea();
		...
		dialogueLayoutRefreshRequested = true;
		if (dialogueLayoutRefreshRoutine == null && isActiveAndEnabled)
			dialogueLayoutRefreshRoutine = StartCoroutine(RefreshDialogueLayoutWhenSettled());
	}

	IEnumerator RefreshDialogueLayoutWhenSettled()
	{
		// TMP/LayoutGroup finishes its own dirty pass at the end of the current frame.
		// Refresh once on the next frame instead of polling the complete dialogue layout in LateUpdate.
		yield return null;

		while (dialogueLayoutRefreshRequested && isActiveAndEnabled)
		{
			dialogueLayoutRefreshRequested = false;

			if (keepDialoguePanelOnScreen &&
				dialoguePanel != null &&
				dialoguePanel.gameObject.activeInHierarchy)
			{
				applyingDeferredDialogueLayout = true;
				try
				{
					ClampDialoguePanelToSafeArea();
				}
			...
			if (dialogueLayoutRefreshRequested)
				yield return null;
		}
	...
	{
		if (!Application.isPlaying || applyingDeferredDialogueLayout || !isActiveAndEnabled)
			return;
	...
	void ApplyActiveDialogueTextLayout()
	{
		...
		float targetY = dialogueTextBaseAnchoredPosition.y + activeStoryUiStyle.BodyTextOffsetY;
		if (!Mathf.Approximately(textPosition.y, targetY))
		{
			textPosition.y = targetY;
	...
	void ApplyActiveDialogueTextFont()
	{
		...
		InvalidateDialoguePreferredSize();
		FindDialogueTextGrowDownLock()?.MarkDirty();
	}
	...
	void ApplyActiveDialoguePanelRect()
	{
		...
		bool changed = false;
		Vector2 targetPosition = activeStoryUiStyle.DialoguePanelAnchoredPosition;
		Vector2 targetSize = activeStoryUiStyle.DialoguePanelSizeDelta;

		// Auto-height owns the runtime Y size. Resetting it to the style base height on every
		// clamp dirties the whole layout, only for AutoHeight to set it back a few lines later.
		if (Application.isPlaying &&
			activeStoryUiStyle.OverrideDialoguePanelAutoHeight &&
			activeStoryUiStyle.DialoguePanelAutoHeight)
		{
			targetSize.y = dialoguePanel.sizeDelta.y;
		}

		if ((dialoguePanel.anchoredPosition - targetPosition).sqrMagnitude > 0.0001f)
		{
			dialoguePanel.anchoredPosition = targetPosition;
			changed = true;
		}

		if ((dialoguePanel.sizeDelta - targetSize).sqrMagnitude > 0.0001f)
		{
			dialoguePanel.sizeDelta = targetSize;
			changed = true;
		}
	...
	void ApplyActiveCharacterNameLayout()
	{
		...
		if ((characterNameRect.anchoredPosition - position).sqrMagnitude > 0.0001f)
	...
	void InvalidateDialoguePreferredSize()
	{
		dialoguePreferredSizeVersion++;
	}

	Vector2 GetCachedDialoguePreferredSize(float textWidth)
	{
		if (dialogueText == null)
			return Vector2.zero;

		if (cachedDialoguePreferredSizeVersion == dialoguePreferredSizeVersion &&
			Mathf.Abs(cachedDialoguePreferredWidth - textWidth) < 0.01f)
		{
			return cachedDialoguePreferredSize;
		}

		cachedDialoguePreferredWidth = textWidth;
		string currentText = dialogueText.text ?? "";
		StoryTextLayoutLock growDownLock = FindDialogueTextGrowDownLock();
		if (growDownLock == null ||
			!growDownLock.TryGetLastPreferredTextSize(currentText, textWidth, out cachedDialoguePreferredSize))
		{
			cachedDialoguePreferredSize = dialogueText.GetPreferredValues(currentText, textWidth, Mathf.Infinity);
		}

		cachedDialoguePreferredSizeVersion = dialoguePreferredSizeVersion;
		return cachedDialoguePreferredSize;
	}
	...
	bool ApplyActiveDialoguePanelAutoHeight()
	{
		...
		Vector2 preferredTextSize = GetCachedDialoguePreferredSize(textWidth);
	...
	public bool ShowChoice(ChoiceNode node)
	{
		...
		for (int i = 0; i < node.options.Count; i++)
		{
			...
			if (!ChoiceOptionVisibility.IsVisible(option))
				continue;
	...
	static void EnsureWardrobeCategoryTabsActive(WardrobeHeroSetupPage setupPage)
	{
		if (setupPage == null)
			return;

		WardrobeCategoryTabs[] tabs = setupPage.GetComponentsInChildren<WardrobeCategoryTabs>(true);
		for (int i = 0; i < tabs.Length; i++)
		{
			WardrobeCategoryTabs tab = tabs[i];
