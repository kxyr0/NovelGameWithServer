...
public sealed class SavePathResolver
{
	...
	public const string RetryFolderName = "retry";
	...
	public string GetRetryChapterCheckpointPath(
	string storyId,
	int slot,
	int chapterIndex)
	{
		string directory = GetRetryStoryDirectory(
			storyId,
			slot);
		...
		return Path.Combine(
			directory,
			"chapter_" +
			Mathf.Max(0, chapterIndex) +
			SaveFileExtension);
	}

	public string GetRetrySeasonCheckpointPath(
		string storyId,
		int slot,
		int seasonIndex)
	{
		...
		return Path.Combine(
			directory,
			"season_" +
			Mathf.Max(0, seasonIndex) +
			SaveFileExtension);
	}

	public string GetRetryStoryDirectory(
		string storyId,
		int slot)
	{
		return Path.Combine(
			_rootDirectory,
			RetryFolderName,
			SafeFilePart(storyId),
			"slot_" + ClampSlot(slot));
	}
